...
async function main() {
  ...
  const header = [
    `# Uyghur words the published library uses that the desktop dictionary lacks.`,
    `# Generated by scripts/build-vocabulary.mjs from ${books} published books.`,
    `# Admitted at >=${MIN_TOTAL} occurrences across >=${MIN_BOOKS} different books, after rejecting`,
    `# Arabic quotation and anything the morphology already accepts.`,
    `#`,
    `# This list is meant to be READ. Delete a line to keep a word out of the`,
    `# dictionary; the next build will honour that.`,
    `# Format: word<TAB>occurrences<TAB>books`,
  ].join("\n");
  await writeFile(
    join(outDir, "vocabulary.txt"),
    `${header}\n${admitted.map((row) => `${row.word}\t${row.total}\t${row.books}`).join("\n")}\n`,
    "utf-8",
  );

  console.log(`\nadmitted:  ${admitted.length.toLocaleString("en-US")} words`);
  ...
  console.log(`\nwritten to data/spellcheck/vocabulary.txt — review it before shipping`);
  console.log(`\ntop admissions:`);
  for (const row of admitted.slice(0, 30)) {
    console.log(`  ${row.word.padEnd(18)} ${String(row.total).padStart(6)}x  ${row.books} books`);
  }
}
