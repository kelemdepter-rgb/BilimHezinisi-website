...
description: >-
  Audit, review and stress-test the WEB edition of «بىلىم خەزىنىسى» (Bilim
  Hezinisi) — the live Uyghur digital library at bilimhezinisi.com (formerly
  bilim-hezinisi-website.vercel.app, which now redirects there),
  repo folder BilimHezinisi-website. Use this skill WHENEVER the user wants the
  site examined rather than changed: find bugs, find what is missing, review
  quality, check speed, check mobile behaviour, check security or RLS, check
  free-tier cost, judge whether a feature is worth building, or produce a
  findings report and the numbered PROMPT-N.md files that Claude Code will
  later execute. Trigger on short or indirect requests too — "review the site",
  "what is wrong with it", "is this any good", "find the problems", "what should
  I build next", or Uyghur like «سايتنى تەكشۈرۈپ چىق»، «خاتالىقلىرىنى تاپ»،
  «يېتەرسىزلىكىنى ئېيت»، «ئىلغارلاشتۇرۇش تەكلىپى بەر»، «نېمە قىلساق بولىدۇ».
  This skill NEVER writes application code — it investigates and writes prompts.
  Do NOT use it for actually building or deploying (that is bilim-web), or for
  the desktop app (bilim-desktop) or the Android app (bilim-apk).
---
